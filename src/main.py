...
STORE_PATH = './trajectories_dir'

os.makedirs(os.path.abspath(STORE_PATH), exist_ok=True)

flags.DEFINE_integer('num_episodes', 1000, 'Number of episodes to log.')
flags.DEFINE_string('trajectories_dir', STORE_PATH,
...
def record_data(unused_argv):
    ...
    dataset_config = tfds.rlds.rlds_base.DatasetConfig(
        ...
        action_info=tfds.features.Tensor(
            shape=(7,),
            dtype=np.float64,
            doc='Actions coming from Kinova arm driver.',
        ),
    ...
    with envlogger.EnvLogger(
        ...
        logging.info('Generating RLDS database...')

        timestep = env.reset()
        while not timestep.last():
            action = np.random.rand(7)  # action should come from OpenVLA
            timestep = env.step(action)

        logging.info('Done generating RLDS database.')
